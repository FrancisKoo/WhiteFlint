...
void DX::DeviceResources::CreateDeviceIndependentResources()
{
	...
	check_hresult(
		DWriteCreateFactory(
			...
			(::IUnknown**)reinterpret_cast<winrt::Windows::Foundation::IUnknown**>(m_dwriteFactory.put())
		)
	);
...
void DX::DeviceResources::CreateDeviceResources()
{
	...
	check_hresult(
		device.as(IID_PPV_ARGS(&m_d3dDevice))
	);

	check_hresult(
		context.as(IID_PPV_ARGS(&m_d3dContext))
	);

	// Create the Direct2D device object and a corresponding context.
	com_ptr<IDXGIDevice3> dxgiDevice;
	check_hresult(
		m_d3dDevice.as(IID_PPV_ARGS(&dxgiDevice))
	);
...
void DX::DeviceResources::CreateWindowSizeDependentResources()
{
	...
	else
	{
		...
		check_hresult(
			swapChain.as(IID_PPV_ARGS(&m_swapChain))
		);
		...
		m_swapChainPanel.Dispatcher().RunAsync(CoreDispatcherPriority::High, [=]()
			{
				...
				check_hresult(
					get_unknown(m_swapChainPanel)->QueryInterface(IID_PPV_ARGS(&panelNative))
				);
	...
	check_hresult(
		m_swapChain.as(IID_PPV_ARGS(&spSwapChain2))
	);
...
void DX::DeviceResources::ValidateDevice()
{
	...
	com_ptr<IDXGIDevice3> dxgiDevice;
	check_hresult(m_d3dDevice.as(IID_PPV_ARGS(&dxgiDevice)));
